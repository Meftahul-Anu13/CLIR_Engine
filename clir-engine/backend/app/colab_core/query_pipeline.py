...
def cached_translate(text: str, src: str, tgt: str) -> str:
    ...
    if not cleaned:
        return ""
    tokenizer, model = TRANSLATOR._load(src, tgt)
...
class QueryProcessor:
    ...
    def process(self, raw_query: str) -> Dict:
        ...
        q_en = normalized if detected_lang == "en" else ""
        q_bn = normalized if detected_lang == "bn" else ""
        translation_error = None
        try:
            if detected_lang == "en":
                translated = cached_translate(normalized, "en", "bn")
                if translated and not BN_RANGE.search(translated):
                    raise ValueError("Translation returned no Bangla characters")
                q_bn = translated
            else:
                q_en = cached_translate(normalized, "bn", "en")
        except Exception as exc:
            translation_error = repr(exc)
            if detected_lang == "en":
                q_bn = ""
            else:
                q_en = ""
        ...
        for mapping in named_entities:
            if detected_lang == "en":
                q_bn = f"{q_bn} {mapping['tgt']}".strip()
            else:
                q_en = f"{q_en} {mapping['tgt']}".strip()

        if detected_lang == "en" and not q_bn.strip():
            q_bn = " ".join(m["tgt"] for m in named_entities).strip()
        if detected_lang == "bn" and not q_en.strip():
            q_en = " ".join(m["tgt"] for m in named_entities).strip()
