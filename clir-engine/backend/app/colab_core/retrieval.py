...
class SemanticRetriever:
    def __init__(self, dataset: DatasetStore, index_store: IndexStore, model_name: str):
    ...
    def search(
        ...
        for doc_id, score in zip(idxs, scores):
            ...
            if allowed_languages and lang not in allowed_languages:
                continue
            raw_scores[int(doc_id)] = float(score)
            results[int(doc_id)] = float((score + 1.0) / 2.0)
...
class RetrievalManager:
    def __init__(
        ...
        self.semantic = SemanticRetriever(dataset, index_store, cfg["semantic_model"])
    ...
    def search(
        ...
        queries = {v for v in query_bundle["query_variants"].values() if v}
        for expansion in query_bundle.get("expansions", []):
            if expansion:
                queries.add(expansion)
        if not queries:
            queries.add(query_bundle.get("normalized", query_bundle.get("original", "")))
        ...
        semantic_raw = getattr(self.semantic, "last_raw_scores", {})
        ...
        for doc_id, score in ordered_ids:
            ...
            if debug:
                record["debug"]["semantic_raw"] = round(semantic_raw.get(doc_id, 0.0), 4)
        ...
        return {
            ...
            "queries_used": queries if debug else None,
        }
